...
class TestCurve:
    ...
    def test_main_axis(self):
        ...
        """
        assert self.curve.features["main_axis"]['axe'] == "x"
        assert self.curve.features["main_axis"]['sign'] == "+"

    def test_correction_baseline(self):
        """
        test the detection of the baseline
        """
        assert float(self.curve.features["baseline_origin_press (N)"]) * 1e12 < float(self.curve.features['baseline_corrected_press (pN)'])
        
    def test_corrected_std(self):
        """
        test the corrected standard deviation
        """
        assert float(self.curve.features['std_origin_press (N)']) * 1e12 < float(self.curve.features['std_corrected_press (pN)'])
