...
class Controller:
    ...
    def create_dict_curves(self, methods, list_files=None):
        ...
        self.dict_type_files = {"txt": 0, "jpk": 0, "NC": 0, "PB": 0, "INC": 0, "DP": 0}
        ...
        for index_file in range(0, len(files), 1):
            ...
            if filename not in self.dict_curve:
                ...
                try:
                    ...
                    self.problematic_curve(files[index_file], type_file, message, error)
                    self.dict_type_files["PB"] += 1
                if check_incomplete:
                    self.list_file_imcomplete.add(files[index_file].split(sep)[-1])
                ...
                if new_curve is not None:
                    if new_curve.check_incomplete:
                        ...
                        try:
                            error = new_curve.analyzed_curve(methods, False)
                            if self.view.check_logger and error is not None:
                                self.logger.info(
                                    "###########################################"
                                )
                                self.logger.info(new_curve.file)
                                self.logger.info(
                                    "###########################################"
                                )
                            ...
                            new_curve.features["type"] = new_curve.features[
                                "automatic_type"
                            ]
            ...
            if not isinstance(self.view, MockView):
    ...
    def open_file(file, name_file, threshold_align, pulling_length=50):
        ...
        with open(file, "r") as file_study:
            ...
            check_incomplete = Controller.check_file_incomplete(
                lines, header["header_global"], nb_segments
            )
            if check_incomplete:
                ...
                while num_segment < nb_segments:
                    segment = Controller.management_data(
                        file_study, nb_segments, num_segment, header["header_global"]
                    )
                    if num_segment < nb_segments - 1:
                        settings_segment = "settings.segment." + str(num_segment + 1)
                        if (
                            header["header_global"][settings_segment + ".style"]
                            != "motion"
                        ):
                            if (
                                float(
                                    header["header_global"][
                                        settings_segment + ".duration"
                                    ]
                                )
                                == 0.0
                            ):
                ...
                new_curve = Curve(
                    file_curve, title, header, dict_segments, pulling_length
                )
    ...
    def create_object_curve(file, name_file, threshold_align, pulling_length=50):
        ...
        if check_incomplete:
            ...
            for segment in new_jpk.segments:
                ...
                for index_column in range(0, len(columns), 1):
                    if columns[index_column] == "t":
                        ...
                        dataframe["seriesTime"] = dataframe["time"].add(
                            time_end_segment + time_step
                        )
                        if segment.index == 0:
                            time_step = dataframe["time"][1]
                        time_end_segment = dataframe["seriesTime"][
                            len(dataframe["seriesTime"]) - 1
                        ]
                ...
                if (
                    float(
                        new_jpk.headers["header_global"][
                            "settings.segment." + str(num_segment) + ".duration"
                        ]
                    )
                    == 0.0
                ):
            ...
            new_curve = Curve(
                file_curve, title, new_jpk.headers, dict_segments, pulling_length
            )
                    ...
                    if "distance" in segment.corrected_data:
                        if segment.header_segment["segment-settings.style"] == "motion":
                            ...
                            if segment.name == "Press":
                                ...
                                i = len(labels) - 1
                                while i >= 0:
                                    if labels[i] == "smooth" or labels[i].startswith(
                                        "fit"
                                    ):
    ...
    def plot_distance(self, curve, segment, ax, graph_position):
        ...
        ax.plot(distance_data, y_smooth, color="#80cdc1", label="smooth")
        index_x_0 = 0
        if segment.name == "Press":
            ...
            legend_threshold = (
                f"{curve.features['tolerance']} x STD = +/-{calcul_threshold:.2f}pN"
            )
            ax.plot(
                distance_data,
                threshold_press,
            ...
            ax.plot(
                distance_data, threshold_press_neg, color="blue", alpha=0.5, ls="-."
            )
            ...
            ax.plot(
                distance_data[index_max],
                force_data[index_max],
                color="#1b7837",
                marker="o",
                ls="None",
                label="min press",
            ...
            calcul_threshold = (
                curve.features["tolerance"]
                * curve.graphics["threshold_pull"][0]
                / curve.features["tolerance"]
            )
            legend_threshold = (
                f"{curve.features['tolerance']} x STD = +/-{calcul_threshold:.2f}pN"
            ...
            if "distance_fitted_classification_release" in curve.graphics:
                ax.plot(
                    curve.graphics["distance_fitted_classification_release"],
                    curve.graphics["fitted_classification_release"],
                    label="fit classification release",
                )
            if "distance_fitted_classification_max" in curve.graphics:
                ax.plot(
                    curve.graphics["distance_fitted_classification_max"],
                    curve.graphics["fitted_classification_max"],
                    label="fit classification max",
                )
            if "distance_fitted_classification_return_endline" in curve.graphics:
                ax.plot(
                    curve.graphics["distance_fitted_classification_return_endline"],
                    curve.graphics["fitted_classification_return_endline"],
                    label="fit classification return",
                )
            if "distance_fit_classification_transition" in curve.graphics:
                ax.plot(
                    curve.graphics["distance_fit_classification_transition"],
                    curve.graphics["fit_classification_transition"],
                    label="fit classification transition",
                )
            elif "distance_fitted_classification_max_transition" in curve.graphics:
                ax.plot(
                    curve.graphics["distance_fitted_classification_max_transition"],
                    curve.graphics["fitted_classification_max_transition"],
                    label="fit classification transition",
                )
            if self.view.methods["optical"] == "Correction":
            ...
            if "point_return_manual" in curve.features:
                ...
                ax.plot(
                    distance_data[index_return],
                    y_smooth[index_return],
                    color="#50441b",
                    marker="P",
                    ls="None",
                    label="point return manual",
                )
            ...
            if "point_transition_manual" in curve.features:
                ...
                ax.plot(
                    distance_data[index_transition],
                    force_data[index_transition],
                    marker="P",
                    ls="None",
                    color="#1E90FF",
                    label="point transition manual",
                )
            elif (
                "transition_point" in curve.features
                and curve.features["transition_point"]["index"] != "NaN"
            ):
                index_trasition = curve.features["transition_point"]["index"]
                ax.plot(
                    distance_data[index_trasition],
                    force_data[index_trasition],
                    marker="o",
                    ls="None",
                    color="#1E90FF",
            ...
            if "type" in curve.features:
                ...
                if (
                    curve.features["automatic_type"] != "NAD"
        ...
        if index_x_0 != 0:
            ax.plot(
                distance_data[index_x_0],
                force_data[index_x_0],
                color="#762a83",
                marker="o",
                ls="None",
                label="contact",
        ...
        ax.set_ylim(
            curve.features["force_min_curve"]["value"] - 2,
    ...
    def plot_time(curve, fig, graph_position):
        ...
        ax1.plot(
            data_total["seriesTime"],
            np.zeros(len(data_total[main_axis + "Signal1"])),
            color="green",
            alpha=0.75,
        )
        ax1.plot(
        ...
        if "point_transition_manual" in curve.features:
            index_transition = curve.features["point_transition_manual"]["index"]
            ax1.plot(
                time_data_pull[index_transition],
                force_data_pull[index_transition],
                marker="P",
                ls="None",
                color="black",
                label="point transition manual",
            ...
            index_trasition = curve.features["transition_point"]["index"]
            ax1.plot(
                time_data_pull[index_trasition],
                force_data_pull[index_trasition],
                marker="o",
                ls="None",
                color="#1E90FF",
                label="transition point",
        ...
        if "point_return_manual" in curve.features:
            ...
            ax1.plot(
                time_data_pull[index_return_endline],
                force_data_pull[index_return_endline],
                marker="P",
                ls="None",
                color="#50441b",
                label="point return manual",
            ...
            ax1.plot(
                ...
                ls="None",
                color="#50441b",
                label="return endline",
            )
        ax1.plot(
            data_total["time"][index_contact],
            data_total[main_axis + "Signal1"][index_contact],
            marker="o",
            ls="None",
            color="#762a83",
            label="contact point",
        ...
        ax1.plot(
            data_total["time"][index_min_press],
            data_total[main_axis + "Signal1"][index_min_press],
            marker="o",
            ls="None",
            color="#8b5847",
            label="min press",
        ...
        ax1.plot(
            curve.features["time_min_curve"]["value (s)"],
            curve.features["force_min_curve"]["value"],
            marker="o",
            ls="None",
            label="min curve",
            color="red",
        ...
        ax1.plot(
            time_data_pull[index_release],
            force_data_pull[index_release],
            marker="o",
            ls="None",
            color="#762a83",
            label="release point",
        ...
        if "type" in curve.features:
            if curve.features["type"] != "NAD" and curve.features["type"] != "RE":
                ax1.plot(
                    data_total["seriesTime"][index_max],
                    data_total[main_axis + "Signal1"][index_max],
                    marker="o",
                    ls="None",
                    color="#1b7837",
                    label="max curve",
            ...
            if (
                curve.features["automatic_type"] != "NAD"
                and curve.features["automatic_type"] != "RE"
            ):
                ax1.plot(
                    data_total["seriesTime"][index_max],
                    data_total[main_axis + "Signal1"][index_max],
    ...
    def global_plot(self, n):
        ...
        if len(list_curves_for_graphs) > 0:
            ...
            if main_axis == "x":
                ...
                ax2.plot(
                    data_total["seriesTime"],
                    np.zeros(len(data_total["ySignal1"])),
                    color="black",
                    alpha=0.75,
                )
                ax2.plot(
                ...
                ax2.plot(
                    data_total["seriesTime"],
                    np.zeros(len(data_total["xSignal1"])),
                    color="black",
                    alpha=0.75,
                )
                ax2.plot(
            ...
            ax3.plot(
                data_total["seriesTime"],
                np.zeros(len(data_total["zSignal1"])),
                color="black",
                alpha=0.75,
            )
            ax3.plot(
            ...
            for segment in curve.dict_segments.values():
                if "distance" in segment.corrected_data:
                    ...
                    if not segment.name.startswith("Wait"):
                        ...
                        ax4.plot(
                            segment.corrected_data["distance"],
                            segment.corrected_data[main_axis + "Signal1"],
                            color="#c2a5cf",
                        )
                        ax4.plot(
                            segment.corrected_data["distance"],
                            np.zeros(
                                len(segment.corrected_data[main_axis + "Signal1"])
                            ),
                            color="black",
                            alpha=0.75,
                        )
                        ...
                        ax4.plot(
                            segment.corrected_data["seriesTime"],
                            segment.corrected_data[main_axis + "Signal1"],
                            color="#34b6cf",
                        )
                        position_start_graph = position_end_graph + 1
                    ...
                    ax4.set_ylim(
                        curve.features["force_min_curve"]["value"] - 1,
                        curve.features["force_max_curve"]["value"] + 2,
                    )
                    num_segment += 1
        return fig, curve, check_distance
    ...
    def file_troncated(jpk_object):
        ...
        if expected_nb_segments != nb_segments_completed:
            for index_segment in range(0, expected_nb_segments - 1, 1):
                style = jpk_object.headers["header_global"][
                    "settings.segment." + str(index_segment) + ".style"
                ]
                if style == "pause":
                    duration = float(
                        jpk_object.headers["header_global"][
                            "settings.segment." + str(index_segment) + ".duration"
                        ]
                    )
                    if duration == 0.0:
    ...
    def piechart_alignment(self, ax, name_alignment):
        ...
        ax.pie(
            values_align_auto,
            explode=explode_align,
            autopct=lambda pct: self.make_autopct(pct, dict_align_auto),
            shadow=True,
            startangle=45,
        )
    ...
    def piechart_incomplete(self, ax):
        ...
        dict_incomplete = {
            "INC": f"{percent_incomplete:.2f}",
            "Treat": f"{percent_treat:.2f}",
            "NC": f"{percent_no_conforming_files:.2f}",
        }
        ...
        ax.pie(
            values_incomplete,
            explode=explode_incomplete,
            autopct=lambda pct: self.make_autopct(pct, dict_incomplete),
            shadow=True,
        )
    ...
    def piechart_classification(self, ax, nb_conforming_curves, name_classification):
        ...
        if nb_conforming_curves != 0:
            percent_NAD = dict_type["NAD"] / nb_conforming_curves * 100
            percent_AD = dict_type["AD"] / nb_conforming_curves * 100
            percent_FTU = dict_type["FTU"] / nb_conforming_curves * 100
            percent_ITU = dict_type["ITU"] / nb_conforming_curves * 100
            percent_RE = dict_type["RE"] / nb_conforming_curves * 100
            dict_classification = {
                "NAD": f"{percent_NAD:.2f}",
                "AD": f"{percent_AD:.2f}",
                "FTU": f"{percent_FTU:.2f}",
                "ITU": f"{percent_ITU:.2f}",
                "RE": f"{percent_RE:.2f}",
            }
            values = [
                f"{percent_FTU:.2f}",
            ...
            ax.pie(
                values_auto,
                autopct=lambda pct: self.make_autopct(pct, dict_classification),
                shadow=True,
                startangle=45,
            )
        return ax
    ...
    def piechart_optical_correction(self, ax):
        ...
        dict_correction_percent = {
            "None": f"{percent_no_correction:.2f}",
            "Auto": f"{percent_auto_correction:.2f}",
            "Manual": f"{percent_manual_correction:.2f}",
        }
        values = [
        ...
        ax.pie(
            values_correction,
            autopct=lambda pct: self.make_autopct(pct, dict_correction_percent),
            shadow=True,
            startangle=45,
        )
        return ax
    ...
    def scatter_bilan(self, fig, gs):
        ...
        for curve in self.dict_curve.values():
            if curve.features["type"] in ("AD", "FTU"):
                ax1.plot(
                    curve.features["jump_distance_end_pull (nm)"],
                    curve.features["jump_force_end_pull (pN)"],
                    marker="o",
                    color=color_dict[curve.features["type"]],
                    picker=True,
                    label=curve.file,
                )
                annotations_ax1.append(
                    ax1.annotate(
                        curve.file,
                        xy=(
                            curve.features["jump_distance_end_pull (nm)"],
            ...
            if "slope_fit_classification_transition" in curve.features:
                ax2.plot(
                    curve.features["slope_fit_classification_transition"],
                    curve.features["jump_force_end_pull (pN)"],
                    marker="o",
                    color=color_dict[curve.features["type"]],
                    picker=True,
                ...
                ax2.plot(
                    curve.features["slope_fitted_classification_max_transition"],
                    curve.features["jump_force_end_pull (pN)"],
                    marker="o",
                    color=color_dict[curve.features["type"]],
                    picker=True,
                ...
                annotations_ax2.append(
                    ax2.annotate(
                        ...
                        xy=(
                            curve.features[
                                "slope_fitted_classification_max_transition"
                            ],
                            curve.features["jump_force_end_pull (pN)"],
                        ),
                        xytext=(-50, 10),
        ...
        fig.canvas.mpl_connect(
            "pick_event",
            lambda event: self.click_curve(event, ax1, fig, annotations_ax1, 30, 50),
        )
        fig.canvas.mpl_connect(
            "pick_event",
            lambda event: self.click_curve(event, ax2, fig, annotations_ax2, 0.005, 30),
        )
    ...
    def output_save(self, path_directory):
        ...
        self.output["main_axis"] = (
            self.output["main_axis_sign"] + self.output["main_axis_axe"]
        )
        ...
        liste_labels = [
            ...
            "force_min_press_value",
            "force_min_curve_index",
            "force_min_curve_value",
            "time_min_curve_index",
            "time_min_curve_value (s)",
            "point_release_index",
            "point_release_value",
            "force_max_pull_index",
            "force_max_pull_value",
            "force_max_curve_index",
            "force_max_curve_value",
                ...
                if label not in liste_labels:
                    if label == "type":
                        ...
                        if label.endswith("Wait1 (s)"):
                            liste_labels.insert(
                                liste_labels.index("theorical_freq_Press (Hz)") + 1,
                                label,
                            )
                        else:
                            liste_labels.insert(
                                liste_labels.index("theorical_freq_Pull (Hz)") + 1,
                                label,
                            )
                    else:
        ...
        self.output.rename(
            columns={
                "contact_point_value": "contact_point_value  (pN)",
                "force_min_press_value": "force_min_press_value (pN)",
                "force_min_curve_value": "force_min_curve_value (pN)",
                "force_max_curve_value": "force_max_curve_value (pN)",
                "point_release_value": "point_release_value (pN)",
                "force_max_pull_value": "force_max_pull_value (pN)",
                "point_return_endline_value": "point_return_endline_value (pN)",
            },
            inplace=True,
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--path",
        type=str,
        help="Name of the folder containing the curves",
        required=True,
    )
    parser.add_argument(
        "-o",
        "--output",
        help="Name of the folder where to save the results",
        required=True,
    )
    parser.add_argument(
        "-m",
