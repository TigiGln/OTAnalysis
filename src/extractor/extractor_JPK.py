...
class JPKSegment:
    ...
    :parameters:
         parent_has_shared_header: bool
            True, if JPKFile finds shared header in JPK archive, False otherwise.

    shared_properties: 
        If parent_has_shared_header is True, this parameter needs to hold the dictionary containing the header's contents. Otherwise it is None.
    """
            ...
            if split[0] == "segments":
                if len(split) >= 3:
                    ...
                    # .dat is the extension for data files.
                        ...
                        self.read_segment_data(
                            segment_number, segment, split, fname)
            # else:
